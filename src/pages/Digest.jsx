...
const formatBulletPoints = (summary) => {
  ...
  return summary.split('•').filter(point => point.trim().length > 0);
};
    ...
    >
      ...
      <div className="space-y-4 sm:space-y-6">
        {data.summaries.map((article, index) => {
          const bulletPoints = formatBulletPoints(article.summary);
          return (
            <motion.article
              key={index}
              variants={item}
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-4 border-t border-[#2A2A35]">
                        <span className="px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium bg-[#2A2A35] text-gray-300">
